...
slam.matrix
```
`slam` n'enregistre pas *la forme de la matrice* comme on a pu le voir avec `Matrix` mais l'ensemble du triplet est renseigné dans la fonction :
```{r eval=FALSE, include=TRUE}
slam.matrix$i
slam.matrix$j
slam.matrix$v
...
Les triplets de `Matrix` et `slam` sont similaires. Et il est intéressant de transformer des matrices pleines en matrices creuses à partir de 50% de 0.  
Les matrices creuses prennent donc moins de places et donc peuvent être stocker dans l'environnement R plus facilement et le temps de calcul peut être considérablement réduit.
